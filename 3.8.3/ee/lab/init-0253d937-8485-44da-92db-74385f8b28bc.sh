...
# + 0.9.5 -> the Simulator version
# * danny -> the user name
# * aws-ec2 -> the used cloud provider
...
if [ -d /mnt/ephemeral ] ; then
    if [ -d /mnt/ephemeral/workers ] ; then
        ...
        rm -fr hazelcast-simulator-0.9.5/workers
        sudo mkdir /mnt/ephemeral/workers
        sudo chown -R danny /mnt/ephemeral/workers/
        ln -s /mnt/ephemeral/workers/ hazelcast-simulator-0.9.5/workers
    fi
else
